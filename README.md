...
- **Inbound:** GitHub App webhooks; Slack `/link-github` (U9) and optional
  `/link-slack` (quiet archiving) slash commands.
...
This is off unless `SLACK_OAUTH_CLIENT_ID`, `SLACK_OAUTH_CLIENT_SECRET`, and
`SLACK_USER_TOKEN_ENC_KEY` are all set. When enabled, each user runs `/link-slack`
once to grant the **`channels:write` user scope** (the only user-token scope
required — it lets `conversations.leave` remove them from public channels). The
OAuth redirect URL is `${PUBLIC_URL}/oauth/slack/callback`. Tokens are stored
encrypted at rest (AES-256-GCM); a revoked token is dropped automatically.
