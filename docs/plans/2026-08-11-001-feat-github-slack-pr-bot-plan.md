...
**Reminders**
- R9. A reminder becomes due approximately 12 hours after a review is requested if that review is still pending; it is suppressed once the review is submitted, the review request is removed, or the PR is closed/merged. Reminders are only **delivered** during a configurable daily window (default 05:00–14:00 in `REMINDER_WINDOW_TZ`, default `America/New_York`); one that comes due outside the window waits, still pending, until the window next opens.
...
- **Approach:** On `review_requested`, insert a `reminders` row `due_at = now + 12h`, status `pending`, keyed to (pr, reviewer github id). A timer loop in the long-running process scans for `pending` rows with `due_at <= now` and claims each with an atomic `UPDATE … SET status='sent' WHERE id=? AND status='pending'` (KTD10) — only the caller whose update affects a row proceeds to post, so concurrent scanners (multi-instance or rolling-deploy overlap) can't double-post. Before posting, re-check that the review is still outstanding and the PR is still open, then post a reminder into the channel mentioning the reviewer. Any `pull_request_review.submitted` from that reviewer, `review_request_removed`, or PR close/merge cancels matching `pending` rows (KTD4 idempotency). A scan whose wall-clock time (in the configured IANA time zone) is outside the delivery window returns early **before** claiming any row, so off-hours reminders stay pending rather than being consumed. Scan interval, the 12h window, and the delivery-hours window (start/end hour + time zone) are configurable.
